 ...
 * service.AbstractUserDetailService                | 用户信息加载
 * service.AbstractCreatedAdminService              | 创建 admin 用户的接口
...
不过这里有个注意的地方, <strong>该接口只能被 ADMIN 角色访问.</strong>
...
POST /authorization/add_role
    ...
    body:
        ...
        description: ${这是个给人看的字段, 不能重复定义}
```
与 `/authorization/add_permission` 接口一样, 该接口也是只能被 ADMIN 角色访问.
...
POST /authorization/grant_role_to/{authorityId}
	...
	params:
		authorityId: ${这里传个用户的 authorityId}
...
GET /admin/create
```
这个接口是创建 admin 账户的, 在创建前请确保已实现 `top.itfinally.security.service.AbstractCreatedAdminService` 接口.
...
3. 再一次用 curl 调用 get /admin/create 创建超级管理员账户
...
Spring 在多实现的情况下优先使用你的实现, 否则 Spring 容器会直接报错.</strong>
 ...
 * 字典对象( MapResponseVoBean )
 ...
 当然了, 已经存在的字段还是不能改的, 除非前端不打算统一规范或者是 JavaScript 这种弱类型语言.
...
1. 通过 账户/密码 获取用户实体
2. 通过用户对应的 authorityId 获取用户信息
3. 通过这个 authorityId 获取对应的角色及其权限

其中 UserDetailService implement 这一步就是需要继承 `top.itfinally.security.service.UserDetailService` 并实现的类,啰啰嗦嗦说那么多就是为了这个.

如果是自己重新实现( 多数情况下都是 ) UserDetailService, 意味着对应的实体也要重新实现, 
对应的用户实体是 `top.itfinally.security.repository.po.AbstractUserDetailsEntity`.

用户注册的流程大概如下:
```
auth = new Authority();
user = new User().setAuthorityId( auth.getId() );
userDetailMapper.save( user );
userAuthMapper.save( auth );
```

以上是一段伪代码, 就是说新增一个用户分为两步
1. 创建用户实体 ( user )
2. 创建用户信息实体 ( authority )

#### 最后
其实最好还是对 spring-security 有个感性的认知, 毕竟这里不是 spring-security 教程, 
能提醒的一个关键点就是无论 ss 看起来多么复杂, 一堆抽象定义, 实际本身是通过责任链来完成一系列任务的, 列一下当初查阅过比较有用的资料:
