...
public final class CollectionUtils {
  ...
  public static final class ZipMethod implements Iterable<Item> {
    ...
    private ZipMethod( boolean isQuick, Collection<?>[] targets ) {
      ...
      this.targets = Arrays.stream( targets ).map( ( Function<Collection<?>, ? extends ArrayList<?>> ) ArrayList::new ).collect( toList() );
  ...
  public static <T> List<T> union( @NotNull Collection<T> src, @NotNull Collection<T>... targets ) {
    ...
    return union(
        Stream.concat( src.stream(), targets[ 0 ].stream() ).collect( toSet() ),
  ...
  public static <T> List<T> intersection( @NotNull Collection<T> src, @NotNull Collection<T>... targets ) {
    ...
    return intersection( new HashSet<>( src ), Stream.empty(), targets ).distinct().collect( toList() );
  }
  ...
  public static <T> List<T> complement( @NotNull Collection<T> src, @NotNull Collection<T>... targets ) {
    ...
    return complement( new HashSet<>( src ), Stream.empty(), targets ).distinct().collect( toList() );
  }
  ...
  public static <T> List<T> merge( @NotNull Collection<T>... targets ) {
    ...
    return merge( targets, Stream.empty() ).collect( toList() );
  }
  ...
  public static <T> List<T> removeIf( @NotNull Collection<T> collection, @NotNull Function<T, Boolean> action ) {
    ...
    return collection.stream().filter( item -> !action.apply( item ) ).collect( toList() );
  }
}
